 ...
 *
 * Displays authentication buttons for all enabled OAuth providers
 * Automatically adapts based on available providers from Strategy Manager
 */
export function OAuthButtons({ providers, callbackUrl = "/dashboard" }: OAuthButtonsProps) {
    ...
    // Filter out credentials provider (we handle that separately)
...
function OAuthButton({ provider, isLoading, onClick }: OAuthButtonProps) {
    const getProviderStyles = (providerId: string) => {
        ...
        return styles[providerId as keyof typeof styles] || styles.default
    }
    ...
    return (
        <button
            ...
            disabled={isLoading}
            className={`
                w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl font-semibold
        ...
        >
            ...
            <span>
                {isLoading ? "Connexion en cours..." : `Continuer avec ${provider.name}`}
            </span>
        </button>
    )
}
