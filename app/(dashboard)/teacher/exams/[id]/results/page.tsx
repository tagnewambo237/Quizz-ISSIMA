            ...
            <div className="min-h-screen bg-gray-50/50 dark:bg-gray-900/50 p-6 lg:p-10">
                <div className="max-w-7xl mx-auto space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {[
                            { label: "Moyenne", value: `${stats.averageScore}%`, icon: Trophy, color: "text-amber-500", bg: "bg-amber-500/10", border: "border-amber-200 dark:border-amber-800" },
                            { label: "Taux de Réussite", value: `${stats.passRate}%`, icon: CheckCircle2, color: "text-emerald-500", bg: "bg-emerald-500/10", border: "border-emerald-200 dark:border-emerald-800" },
                            { label: "Temps Moyen", value: `${stats.averageTimeSpent}m`, icon: Clock, color: "text-blue-500", bg: "bg-blue-500/10", border: "border-blue-200 dark:border-blue-800" },
                            { label: "Total Participants", value: stats.totalAttempts, icon: Users, color: "text-purple-500", bg: "bg-purple-500/10", border: "border-purple-200 dark:border-purple-800" },
                        ].map((stat, i) => (
                            <motion.div
                                key={stat.label}
                                initial={{ opacity: 0, y: 20 }}
                        ...
                        >
                            ...
                            <div className="space-y-4">
                                {topPerformers.length === 0 ? (
                                    <p className="text-sm text-gray-500 text-center py-4">Aucune tentative complétée</p>
                                ) : topPerformers.map((attempt, i) => (
                                    <div key={attempt.id} className="flex items-center gap-4 p-3 rounded-2xl hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                        ...
                                        <div className="text-right">
                                            <p className="font-bold text-emerald-600 dark:text-emerald-400">{attempt.percentage}%</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
                            ...
                            <div className="flex gap-2">
                                <Input
                                    ...
                                    className="h-9 w-64 bg-gray-50 border-transparent focus:bg-white transition-all rounded-xl"
                                />
                            </div>
                        </div>
                                    ...
                                    {filteredAttempts.length === 0 ? (
                                        <tr>
                                            <td colSpan={5} className="px-6 py-12 text-center text-gray-500">
                                                {attempts.length === 0 ? "Aucune tentative pour cet examen" : "Aucun résultat trouvé"}
                                            </td>
                                        </tr>
                                        ...
                                        <tr key={attempt.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    <div className="w-16 h-2 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                                                        <div
                                                            className={cn("h-full rounded-full", attempt.percentage >= 80 ? "bg-emerald-500" : attempt.percentage >= 50 ? "bg-blue-500" : "bg-red-500")}
                                            ...
                                            <td className="px-6 py-4 text-gray-500">{attempt.timeSpentFormatted}</td>
