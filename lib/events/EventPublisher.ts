...
export class EventPublisher {
  ...
  subscribe(observer: IObserver): void {
    ...
    for (const eventType of interestedEvents) {
      ...
      if (!observers.includes(observer)) {
        ...
        console.log(
          `[EventPublisher] ${observer.getName()} subscribed to ${eventType}`
        );
      }
    }
  }
  ...
  unsubscribe(observer: IObserver): void {
    for (const [eventType, observers] of this.observers.entries()) {
      ...
      if (index > -1) {
        ...
        console.log(
          `[EventPublisher] ${observer.getName()} unsubscribed from ${eventType}`
        );
      }
    }
  }
  ...
  async publish(event: Event): Promise<void> {
    ...
    console.log(
      `[EventPublisher] Publishing ${event.type} to ${observers.length} observers`
    );
  ...
  private async notifyObserver(
    ...
    try {
      ...
      console.log(
        `[EventPublisher] ${observer.getName()} notified successfully`
      );
    } catch (error) {
      console.error(
